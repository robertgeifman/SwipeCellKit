...
extension MailCollectionViewController: SwipeCollectionViewCellDelegate {
	func collectionView(_ collectionView: UICollectionView, editActionsForItemAt indexPath: IndexPath, for orientation: SwipeActionsOrientation) -> [SwipeAction]? {
		...
		if orientation == .left {
			...
			let read = SwipeAction(style: .default, title: nil) { _, indexPath in
			...
			let delete = SwipeAction(style: .destructive, title: nil) { _, indexPath in
			...
			let more = SwipeAction(style: .default, title: nil) { _, _ in
...
class MailCollectionViewCell: SwipeCollectionViewCell {
	...
	var unread = false {
		didSet {
			indicatorView.transform = unread ? CGAffineTransform.identity : CGAffineTransform(scaleX: 0.001, y: 0.001)
		}
	}
